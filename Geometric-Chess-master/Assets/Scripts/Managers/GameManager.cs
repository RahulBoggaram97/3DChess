...
public class GameManager : Singleton<GameManager> {
	...
	private Grid grid;
	[SerializeField]
	private com.impactional.chess.gridPuzzle gridPuz;
	[SerializeField]
	...
	IEnumerator init() {
		...
		//wait for nodes
			...
			//wait for pieces
			while (!grid.ArePiecesSpawned) yield return null;
		//wait till pieces are spawned
	...
	public void SwitchPlayer() {
		...
		if (currentPlayer == p2) {
			currentPlayer = p1;
			whiteTurn = true;
			mainCamera.GetComponent<com.impactional.chess.SwitchAngle>().SwitchCamera(PlayerType.P1);
		} else if (currentPlayer == p1) {
			currentPlayer = p2;
			whiteTurn = false;
			mainCamera.GetComponent<com.impactional.chess.SwitchAngle>().SwitchCamera(PlayerType.P2);
		} else {
			currentPlayer = p1;
			whiteTurn = true;
		}
