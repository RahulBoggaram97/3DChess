...
{
    public class GameManagerMulti : MonoBehaviourPun
    {
		public bool destroyOnLoad;
		protected static bool _destroyOnLoad;

		private static GameManagerMulti instance;

		public static GameManagerMulti Instance
		{
			get
			{
				GameManagerMulti foundObject = FindObjectOfType<GameManagerMulti>();
		...
		private GcPlayerMulti p1 = new GcPlayerMulti(MultiPlayerType.white);
		private GcPlayerMulti p2 = new GcPlayerMulti(MultiPlayerType.black);
		private GcPlayerMulti currentPlayer;

		private GameStateMulti gameState;


		private nodeMulti nodeThatJustGotHit;
		...
		public GcPlayerMulti PlayerOponent
		{
			get
			{
		...
		public GameStateMulti GameState
		{
			get { return gameState; }
		}

		public GcPlayerMulti CurrentPlayer
		{
		...
		public GcPlayerMulti P1
		{
			get { return p1; }
		}

		public GcPlayerMulti P2
		{
			get { return p2; }
		}
		...
		public nodeMulti NodeThatJustGotHit
        {
			get { return nodeThatJustGotHit; }

            set
            {
				nodeThatJustGotHit = value;
            }
        }
		...
		public void GameManagerInitRPC()
        {
			if (PhotonNetwork.IsMasterClient) photonView.RPC("gameManagerIntiolMethod", RpcTarget.AllBufferedViaServer);
        }

		[PunRPC]
		public void gameManagerIntiolMethod()
        {
			 StartCoroutine(init());
		} 
		...
		IEnumerator init()
		{
			...
			mainCamera.GetComponent<SwitchAngle>().SetCameramulti();
		...
		void Update()
		{
			...
			{
				GameManagerMulti.Instance.GameState.Checkmate();
			...
			{
				GameManagerMulti.Instance.GameState.Stalemate();
			...
			if (whiteTurn)
			{
				...
				{
					whiteTimer = 0;
					GameManagerMulti.Instance.GameState.OutOfTime();
		...
		public void SwitchPlayer()
		{
			...
			//IF checkmate
			if (RulesMulti.HasNoMove())
			{
				inputManagerMulti.Instance.UnHighlightTile();
		...
		public GcPlayerMulti Opponent(GcPlayerMulti player)
		{
		...
		public void GameOver(GcPlayerMulti winner, GameOverType gameOverType)
		{
			AddGame();
			switch (gameOverType)
			{
		...
		public void GrabThePieceGameMan(nodeMulti node)
        {
			
			if (node == null) return;
			if(node.Piece.gameObject.GetPhotonView().IsMine)
			photonView.RPC("grabThePiecRPC", RpcTarget.AllBufferedViaServer, node.transform.position.x, node.transform.position.y, node.transform.position.z);
		}

		[PunRPC]
		public void grabThePiecRPC(float x, float y, float z)
        {
			if (currentPlayer == null) UnityEngine.Debug.Log("current player is null");
            currentPlayer.GrabThePiece(x, y, z);
        }

        public void PlaceThePieceGameMan(nodeMulti node)
        {
            photonView.RPC("PlaceThePieceRPC", RpcTarget.AllBufferedViaServer, node.transform.position.x, node.transform.position.y, node.transform.position.z);
        }

        [PunRPC]
        public void PlaceThePieceRPC(float x, float y, float z)
        {
			if (currentPlayer == null) UnityEngine.Debug.Log("current player is null");
			currentPlayer.PlaceThePiec(x, y, z);
		}
