...
{
    public class gridMulti : MonoBehaviourPun
    {
		...
		public nodeMulti GetNodeAt(int row, int col)
		{
			if (row < 0 || row >= rows || col < 0 || col >= cols) return null;
			return grid[row, col];
		}

		public nodeMulti GetNodeAt(Vector3 pos)
		{
		...
		public void createGridRpc()
        {
			photonView.RPC("CreateGrid", RpcTarget.AllBufferedViaServer);
        }
		...
		public void CreateGrid()
		{
			...
			for (int row = 0; row < rows; row++)
			{
				for (int col = 0; col < cols; col++)
				{
					...
					nodeMulti dn = go.AddComponent<nodeMulti>();
					dn.row = row;
					dn.col = col;
            ...
            GameManagerMulti.Instance.GameManagerInitRPC();
        }
		...
		IEnumerator SpawnPieces()
		{
			...
			MultiPlayerType p1T = MultiPlayerType.white;
			MultiPlayerType p2T = MultiPlayerType.black;
		...
		public void SpawnPiece(GridCoords coords, GameObject piece, float yRotation, MultiPlayerType playerType)
		{
			nodeMulti pieceNode = GetNodeAt(coords.row, coords.col);
			...
			pieceMulti pieceScript = pieceObject.GetComponent<pieceMulti>();
			...
			Material mat = null;
			GcPlayerMulti player = null;
			switch (playerType)
			{
				case MultiPlayerType.white:
					mat = GameManagerMulti.Instance.PieceP1;
					player = GameManagerMulti.Instance.P1;
					break;
				case MultiPlayerType.black:
					mat = GameManagerMulti.Instance.PieceP2;
					player = GameManagerMulti.Instance.P2;
					break;
			}
			pieceObject.GetComponent<Renderer>().material = mat;
			player.AddPieces(pieceScript);
			pieceScript.PieceMovement = creatorMulti.CreatePieceMovement(pieceScript.MovementType, player, pieceScript);
		...
		public void SpawnPiece(GridCoords coords, GameObject piece, MultiPlayerType playerType)
		{
			SpawnPiece(coords, piece, 0, playerType);
		}

		public List<nodeMulti> GetNeighbours(nodeMulti node)
		{
			List<nodeMulti> neighbours = new List<nodeMulti>();
		...
		public bool IsNearby(nodeMulti nodeA, nodeMulti nodeB)
		{
			for (int row = -1; row <= 1; row++)
			{
				for (int col = -1; col <= 1; col++)
