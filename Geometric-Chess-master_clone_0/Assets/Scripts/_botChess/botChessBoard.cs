...
public class botChessBoard : MonoBehaviour
{
    ...
    //Chess ai
    [Header("Chess ai")]
    public botChessApiCaller aiApiCaller;
    int depth = 20;
   ...
   //Chess bot
   public void GetComMove()
    {
        getCurrentFenPos();
        aiApiCaller.GetMoves(fenString, depth);
    }
    ...
    public void getCurrentFenPos()
    {
            ...
            for (int i = 0; i < TILE_COUNT_X; i++)
            {
               if(chessPieces[i, j] == null)
                ...
                else
                {
                    if(emptySpace != 0)
                    {
                        addCharToFenString(emptySpace.ToString());
                        emptySpace = 0;
                    }

                    //if it is white
                    if(chessPieces[i, j].type == chessPieceTypePuzzle.Pawn)
                    {
                        if(chessPieces[i, j].team == 0)
                        {
                            addCharToFenString("P");
                        }
                    }
                    if (chessPieces[i, j].type == chessPieceTypePuzzle.Rook)
                    {
                        if (chessPieces[i, j].team == 0)
                        {
                            addCharToFenString("R");
                        }
                    }
                    if (chessPieces[i, j].type == chessPieceTypePuzzle.Knight)
                    {
                        if (chessPieces[i, j].team == 0)
                        {
                            addCharToFenString("N");
                        }
                    }
                    if (chessPieces[i, j].type == chessPieceTypePuzzle.Bishop)
                    {
                        if (chessPieces[i, j].team == 0)
                        {
                            addCharToFenString("B");
                        }
                    }
                    if (chessPieces[i, j].type == chessPieceTypePuzzle.Queen)
                    {
                        if (chessPieces[i, j].team == 0)
                        {
                            addCharToFenString("Q");
                        }
                    }
                    if (chessPieces[i, j].type == chessPieceTypePuzzle.King)
                    {
                        if (chessPieces[i, j].team == 0)
                        {
                            addCharToFenString("K");
                        }
                    }
        ...
        Debug.Log("Current fen string is " + fenString);
    }

    public void addCharToFenString(string charcater)
    {
        fenString = fenString + charcater;
    }



    //actuallly making the move

    public void makeComeMove(string move)
    {
        Debug.Log("made the move");
        chessPiecePuzzle comPieceToMove = chessPieces[integerFromChar(move[0]), (move[1] - '1')];

        currentDragging = comPieceToMove;
        availableMoves.Clear();
        ...
        MoveTo(comPieceToMove, integerFromChar(move[2]), (move[3] - '1'));
        currentDragging = null;
    }



    int integerFromChar(char character)
    {
        switch (character)
        {
            case 'a':
